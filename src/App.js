...
const FaceTouchDetector = () => {
  ...
  const drawResults = () => {
    ...
    // Check for bad posture or face touching to add reddish overlay
    let shouldShowRedOverlay = false;
    
    // Check posture
    if (window.currentPoseLandmarks && window.currentPoseLandmarks.length > 0) {
      const { isBad: currentlyBadPosture } = checkPosture(window.currentPoseLandmarks);
      if (currentlyBadPosture) {
        shouldShowRedOverlay = true;
      }
    }
    
    // Check face touching
    if (window.currentFaceLandmarks && window.currentHandLandmarks) {
      const { isTouching } = checkFaceTouch(window.currentFaceLandmarks, window.currentHandLandmarks);
      if (isTouching) {
        shouldShowRedOverlay = true;
      }
    }
    
    // Add reddish overlay if bad posture or face touching detected
